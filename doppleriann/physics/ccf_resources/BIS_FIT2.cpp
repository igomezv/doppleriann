// Author: Y. Zhao (2023), yinan.zhao@unige.ch
#include <iostream>
#include <fstream>
...
// RV,spectrum,err,ones(10000,'d')
    ...
    //aa is the rv bb is the spec, cc is the errors zz is the vecot-1s
    ...
    ifstream input2("run/CCF_data.txt");
    //read the data here:
    ...
    //
    //
    ifstream input3("run/CCF_error.txt");
    //read the data here:
    for(int i = 0; i < vector_size; i++)
    {
    input3 >> p.err[i];
	...
	// printf("%f\t%f\t%f\n", span, x0+x[0], RV_bottom);
    // dd model, c continuum, fabs(k) contrast, span, v0 vrad, fwhm, ee depth, ff bis
    ...
    double *error_ccf_par;
    error_ccf_par = (double*)malloc( 5 * sizeof(double) );
    error_ccf_par[0] = sig_c;
    error_ccf_par[1] = sig_k;
    error_ccf_par[2] = 0;
    error_ccf_par[3] = sig_x0;
    error_ccf_par[4] = sig_fwhm;


    string dataname_model=std::string("run/ccf_model.bin");
    FILE *file_model = fopen(dataname_model.c_str(),"wb");
        fwrite(dd,sizeof(double), vector_size, file_model);
        fclose(file_model);


    string dataname_par=std::string("run/ccf_parameter.bin");
    FILE *file_par = fopen(dataname_par.c_str(),"wb");
        fwrite(ccf_par,sizeof(double), 5, file_par);
        fclose(file_par);

    string dataname_error=std::string("run/ccf_Error_parameter.bin");
    FILE *file_error = fopen(dataname_error.c_str(),"wb");
        fwrite(error_ccf_par,sizeof(double), 5, file_error);
        fclose(file_error);

    string dataname_depth=std::string("run/ccf_depth.bin");
    FILE *file_depth = fopen(dataname_depth.c_str(),"wb");
        fwrite(ee,sizeof(double), len_depth, file_depth);
        fclose(file_depth);

    string dataname_bis=std::string("run/ccf_bis.bin");
