// Author: X. Dumusque (2023), xavier.dumusque@unige.ch.
#include <Python.h>
#include <math.h>
...
static PyObject *gauss_bis (PyObject *self, PyObject *args)
	{
		...
		//printf("C here 1\n");
		if (!PyArg_ParseTuple(args, "O!O!O!O!",&PyArray_Type, &aa, &PyArray_Type, &bb, &PyArray_Type, &cc, &PyArray_Type, &zz)) 
			//le vecteur zz permet juste de donner la taille pour le retour du vecteur bissecteur et depth
			return NULL;
		//printf("C here 2\n");
		//printf("%i\n",PyArray_NDIM(aa));

		x = (double *) (PyArray_DATA(aa) + 0*PyArray_STRIDES(aa)[0]);
		//for (i=0; i<10;i++)
		//	printf("%f\n",x[i]);
		p.y = (double *) (PyArray_DATA(bb) + 0*PyArray_STRIDES(bb)[0]);
		p.err = (double *) (PyArray_DATA(cc) + 0*PyArray_STRIDES(cc)[0]);
		p.n = PyArray_SIZE(aa);		
		double weights[p.n];
		...
		len_fit_tmp = (long)p.n;
        len_fit_ptr = &len_fit_tmp;
		dd = (PyArrayObject *)PyArray_SimpleNewFromData(1,len_fit_ptr, NPY_DOUBLE,p.fit);
		//printf("C here 4\n");
		//printf("%i\n",PyArray_SIZE(dd));
		...
		for (i=0; i<len_depth; i++) {
		  ...
		  bis[i] /= 2.;
		  //printf("%f\t%f\t%f\t%f\t%i\t%i\t%i\n", bis[i], depth[i], p0[i_b], p0[i_r], i_b, i_r,i);
		};
		...
		span = RV_top-RV_bottom;
              ...
              //printf("%d\n",len_depth);
              //printf("%d\n",depth);
              len_depth_tmp = (long)len_depth;
              len_depth_ptr = &len_depth_tmp;
		//dd = (PyArrayObject *)PyArray_SimpleNewFromData(1,(long *)&p.n, NPY_DOUBLE,p.fit);
     		ee = (PyArrayObject *)PyArray_SimpleNewFromData(1,len_depth_ptr, NPY_DOUBLE,depth);
		ff = (PyArrayObject *)PyArray_SimpleNewFromData(1,len_depth_ptr, NPY_DOUBLE,bis);
              //printf("C here 6\n");    
              //printf("len: %li\n",PyArray_SIZE(ee));
              //printf("len: %li\n",PyArray_SIZE(ff));
		// doesn't work: printf("element 0 %d\n",ee);
	...
	//model, constant, ampli, RV, FWHM, sig constant, sig ampli, sig RV, sig FWHM, j, BIS SPAN, depth vector, barycenter at depth, length of depth vector
...
/*static int fit_CCF_traverse(PyObject *m, visitproc visit, void *arg) {
    Py_VISIT(GETSTATE(m)->error);
    return 0;
}

static int fit_CCF_clear(PyObject *m) {
...
PyMODINIT_FUNC PyInit_fit_CCF(void)
{
    ...
    st->error = PyErr_NewException("fit_CCF.Error",NULL,NULL);
    if (st->error ==NULL) {
        Py_DECREF(module);
        INITERROR;
    }
