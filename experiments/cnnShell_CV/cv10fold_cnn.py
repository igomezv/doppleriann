...
prefix_name = f"cnnshellCV10_{n_reso}_{shell_type_str}_{str_spec_types}"
...
def build_folds(n_samples=2036):
    """
    Build 10 folds with sizes [204, 204, 204, 203, 203, 204, 204, 204, 203, 203],
    ...
    fold_sizes = [204, 204, 204, 203, 203, 204, 204, 204, 203, 203]
    ...
    # Coverage / overlap checks (before saving is also fine)
    all_idx = np.concatenate(folds)
    assert all_idx.size == n_samples, "Total indices across folds != n_samples."
    assert np.unique(all_idx).size == n_samples, "Duplicate indices detected across folds."
...
def run_cross_validation():
    ...
    logger.info("10-fold CV + evaluation finished.")
